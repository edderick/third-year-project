...
%Might be useful if I cut abot 4 entires
%\setlength{\nomitemsep}{-\parsep}
...
machine on the network to be globally addressable -- at first this may seem to
be insecure, but a good firewall is far more effective protection than NAT\@.
Even with this many addresses, the University still struggles. This is due at
part to address space wastage, the now deprecated ISS wireless service still
takes up a large proportion of the address space.
...
\nomenclature{RIP}{Routing Information Protocol} \footnote{The routing protocol
...
contents of a RA in the style of a \texttt{radvd} configuration file, an example is shows in Appendix~\ref{radvdump} 
