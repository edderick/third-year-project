...
addresses. Although this is not yet common place, 6\textsuperscript{th} June 2012 was World IPv6
...
The most recent release of this document was on 15\textsuperscript{th} April 2013. Although this
...
26\textsuperscript{th} April 2013, although it is likely a refresh will be released soon.
...
Finally Quagga is a good choice as there were no existing implementations of
autoconfig OSPFv3 based on Quagga, so my contributions would be more useful to
the community.
...
the Quagga routing daemon. My changes consisted of adding $\mysym1,900$ lines of C
code to the \texttt{ospf6d} code base along with $\mysym400$ lines to \texttt{zebra} and
\texttt{lib}. I also modified existing lines. $\mysym600$ lines of code were
...
ULAs are similar in concept to IPv4 Private addresses (e.g. 192.168.1.0).\footnote{Although it is
not recommended to use NAT with IPv6}
...
seconds. The draft recommends that ULAs are deprecated if there are any global
addresses in the system, so as to prevent local IPv6 islands forming -- this is
still an area of debate within the IPv6 community. 
...
the policy table. To overcome this I used \texttt{iproute2}'s \texttt{ip
...
construct. The program  executes as one process, only one thing is being
...
ospf6 database detail}'' can be used. ``\texttt{sh ipv os da de}''  will
perform the same command as it is unambiguous. The aforementioned command will
...
easy to install onto my Netkit testbed -- it required nothing more than
cloning the Git repository and then compiling the fork of Bird on one of the
Netkit VMs. 
...
allows LSAs to be processed by an separate program. 

Bird-Ext-LSA is complimented by hnet-core\footnote{Available at:
\url{https://github.com/fingon/hnet-core}} (Homenet Core), a project that
makes use of Bird-Ext-LSA to implement, among other things, the Autoconfig
drafts\@.  hnet-core is written in the scripting language Lua. Running this
code was a challenge -- I had problems creating the right environment; the
code has  many dependencies. \texttt{Luarocks} helped install some of these.
I did eventually get the code running on my Netkit testbed.
