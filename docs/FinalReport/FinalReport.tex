...
3.2)\cite{rfc4193}, and aims to be as unique as possible; it is based on a hash
of a portion of a combination of the current network time (in Network Time
Protocol (NTP) \nomenclature{NTP}{Network Time Protocol} format), and an
interface's 64-bit MAC address (EUI-64). The algorithm generates a /48 prefix. 
...
\section{Quagga}
Quagga is written in C89, following the GNU coding standards. These rules help
...
for it.  Quagga does not support Microsoft Windows\footnote{Or WinDoze as the
GNU coding standards would put it}.
