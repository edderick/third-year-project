...
become Internet standards and are seen as the highest form of standard the IETF
produces. 

\section{OSPFv3 Autoconfiguration}
\todo{Mention new releases during the project}
...
by enforcing some default values specified in the original OSPF RFCs, and
restricting a few features of OSPF such as allowing only the backbone area to
be used. It then goes on to specify a mechanism for generating Router IDs and
ensuring that there are no conflicts across the network. This is achieved by
the introduction of a new Link State Advertisement (LSA)
...
delegating IPv6 network prefixes from a larger pool of IPv6 prefixes. As well
...
home networking. The draft puts forward a complete architecture for future home
networks, and goes on to give a discussion of many emerging technologies. 

\subsection{OSPF Version 2}
I studied RFC2328\cite{RFC2328} in depth to understand how the OSPF Protocol works. The 
...
RFC5340 defines OSPFv3\cite{RFC5340}. The RFC builds on RFC2328, giving the
read a full undestanding of OSPFv3 by explaining the differences
between OSPFv2 and OSPFv3. 

\subsection{Autoconfiguration of Routers Using a Link State Routing Protocol}
This document \cite{draft-dimitri-zospf-00} is an old draft that had similar aims to the OSPFv3 
...
RFC5250\cite{RFC5250} introduces the opaque LSA\@. An opaque LSA is a generic type of LSA
...
While learning about this area of networking, there were also a few documents
that were either referenced from the more relevent drafts or defined a concept
that had been mentioned. The list of these documents that I read includes:

\begin{itemize} 
  \item {\bf RFC 5838} - OSPFv3-AF (Address Families) 
  \item {\bf RFC 6506} - OSPFv3-AUTH-TRAILER 
  \item {\bf RFC 3630} (4230 \& 5786) - Traffic Engineering Extensions for OSPF 
  \item {\bf RFC 4862} - Stateless address autoconfiguration (SLAAC for IPv6) 
\end{itemize}
...
In general the books were useful for clarifying things that were not very clear
from the RFCs. They were also useful for reinforcing my general understanding
of the protocol, as they gave a similar level overview, written from a slightly
different angle. 

Unfortunately the books did not cover much OSPFv3 because it is a relatively
recent development, and there was predictably no mention at all of more recent
drafts (such as the autoconfig draft) because they are very much on the cutting
edge.
...
However, as we head forward into a world dominated by IPv6, the use of IPv4
addresses will diminish. IPv6 addresses are not such a good candidate for
Router ID for many reasons. Firstly they are not 32 bits, so it would be
difficult to create an identifier known to be unique from this value.  Secondly
an interface is likely to have many IPv6 Addresses. Of these addresses the only
address it is guaranteed to have is a link local address, which is not
...
end users do not have any understanding of how the system they have in place
works, and would not want to touch the configuration anyway. Home users may end
...
\todo{MoSCoW specification of the project}
