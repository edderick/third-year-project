...
\todo{Bring forward the part about CIDR}
...
\section{Routing}
\todo{Appendix might like a layer diagram}
...
\todo{More squished up diagrams would be easier to compare}
...
zero configuration OSPFv3. Along the way I learned a lot. 

\section{Ambiguities in the Drafts}
There are a few areas of the draft that I feel are slightly ambiguous or under
prescriptive. In the Autoconfig draft: 
...
\begin{enumerate}
	\item In section 5.1 the draft says: 
		...
		duplicate Router-ID.'' 
		\end{quote}
		I am unsure of how this should be detected. My implementation checks the
		router's interfaces link local addresses, comparing them to the source
		address of the packet. I am not sure my method is correct, and feel the
		draft should be more prescriptive on this issue.
	...
	\end{quote}
		These parameters include the Router-ID. The draft doesn't seem mention
		what to do if there is a conflict between an automatically generated RID
		and a manually configured one: The two routers decide independently whether
		they need to change. If the auto-configured router had a higher lladr or
		router-hardware fingerprint, then it would assume the other router would
		change. It would seem undesirable for a manually configured parameter to
		change, but if it didn't the conflict would remain.

	\item I don't actually understand why we need to check the neighbour case
		(5.1) at all. Surely it is covered by the non-neighbour case (5.2). I don't
		know if drafts are supposed to justify things, but I would find that
		helpful. 
\end{enumerate}
